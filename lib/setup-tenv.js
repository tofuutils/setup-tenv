...
async function downloadAndExtractCLI (url) {
  ...
  const pathToCLICompressedFile = await tc.downloadTool(url);

  if (!pathToCLICompressedFile) {
    throw new Error(`Unable to download tenv from ${url}`);
  }

  let pathToCLI;

  core.debug(`Extracting tenv CLI ${fileExtension} file`);
  if (os.platform().startsWith('win')) {
    core.debug(`tenv CLI Download Path is ${pathToCLICompressedFile}`);
    const fixedPathToCLIZip = `${pathToCLICompressedFile}.zip`;
    io.mv(pathToCLICompressedFile, fixedPathToCLIZip);
    core.debug(`Moved download to ${fixedPathToCLIZip}`);
    pathToCLI = await tc.extractZip(fixedPathToCLIZip);
  } else {
    pathToCLI = await tc.extractTar(pathToCLICompressedFile);
  }

  core.debug(`tenv CLI path is ${pathToCLI}.`);

  if (!pathToCLI) {
    throw new Error('Unable to extract tenv');
  }
